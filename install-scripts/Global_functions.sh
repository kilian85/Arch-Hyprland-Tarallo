...
install_package_pacman() {
  ...
  if pacman -Q "$1" &>/dev/null ; then
    echo -e "${INFO} ${MAGENTA}$1${RESET} già installato. Salto..."
  else
    ...
    if pacman -Q "$1" &>/dev/null ; then
      echo -e "${OK} Package ${YELLOW}$1${RESET} installato con successo!"
    else
      echo -e "\n${ERROR} ${YELLOW}$1${RESET} installazione fallita. Controlla $LOG. Potrebbe essere necessaria l'installazione manuale."
    fi
  fi
}
...
install_package() {
  if $ISAUR -Q "$1" &>> /dev/null ; then
    ...
    if $ISAUR -Q "$1" &>> /dev/null ; then
      ...
      echo -e "\n${ERROR} ${YELLOW}$1${RESET} installazione fallita :( controlla install.log. Potrebbe essere necessaria l'installazione manuale! Mi dispiace :("
    fi
  fi
}
...
uninstall_package() {
  ...
  if pacman -Qi "$pkg" &>/dev/null; then
    ...
    echo -e "${INFO} Pacchetto $pkg non installato, salto."
  fi
  return 0
}