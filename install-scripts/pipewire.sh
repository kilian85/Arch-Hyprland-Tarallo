...
echo -e "${NOTE} Disabilitazione pulseaudio per evitare conflitti..."
systemctl --user disable --now pulseaudio.socket pulseaudio.service >> "$LOG" 2>&1 || true

# Pipewire
echo -e "${NOTE} Installazione pacchetti ${SKY_BLUE}Pipewire${RESET}..."
...
echo -e "${NOTE} Attivazione servizi Pipewire..."
