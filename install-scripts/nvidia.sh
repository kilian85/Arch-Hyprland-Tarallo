...
# nvidia stuff
printf "${YELLOW} Controllo pacchetti Hyprland in conflitto da rimuovere..${RESET}\n"
if pacman -Qs hyprland > /dev/null; then
  printf "${YELLOW} Hyprland rilevato. Rimozione per installare dalla repo ufficiale...${RESET}\n"
...
printf "${YELLOW} Installazione ${SKY_BLUE}pacchetti Nvidia e header Linux${RESET}...\n"
...
if grep -qE '^MODULES=.*nvidia. *nvidia_modeset.*nvidia_uvm.*nvidia_drm' /etc/mkinitcpio.conf; then
  echo "Moduli Nvidia già presenti in /etc/mkinitcpio.conf" 2>&1 | tee -a "$LOG"
else
...
printf "\n%.0s" {1..1}
printf "${INFO} Ricostruzione ${YELLOW}Initramfs${RESET}...\n" 2>&1 | tee -a "$LOG"
...
if [ -f "$NVEA" ]; then
  printf "${INFO} ${YELLOW}nvidia_drm modeset=1 fbdev=1${RESET} è già presente nel sistema..continuo."
  printf "\n"
else
  printf "\n"
  printf "${YELLOW} Aggiunta opzioni in $NVEA..."
...
if [ -f /etc/default/grub ]; then
    printf "${INFO} Bootloader ${YELLOW}GRUB${RESET} rilevato\n" 2>&1 | tee -a "$LOG"
    ...
    if sudo grep -q "nvidia-drm.modeset=1" /etc/default/grub || sudo grep -q "nvidia_drm.fbdev=1" /etc/default/grub; then
       ...
       printf "${INFO} Configurazione ${YELLOW}GRUB${RESET} rigenerata\n" 2>&1 | tee -a "$LOG"
    fi
...
if [ -f /boot/loader/loader.conf ]; then
    printf "${INFO} Bootloader ${YELLOW}systemd-boot${RESET} rilevato\n" 2>&1 | tee -a "$LOG"
    ...
    if [ "$backup_count" -ne "$conf_count" ]; then
        find /boot/loader/entries/ -type f -name "*.conf" | while read imgconf; do
            ...
            printf "${INFO} Backup creato per systemd-boot loader: %s\n" "$imgconf" 2>&1 | tee -a "$LOG"
        ...
        printf "${OK} Passi aggiuntivi per ${YELLOW}systemd-boot${RESET} completati\n" 2>&1 | tee -a "$LOG"
    else
        printf "${NOTE} ${YELLOW}systemd-boot${RESET} è già configurato...\n" 2>&1 | tee -a "$LOG"
    fi
fi
